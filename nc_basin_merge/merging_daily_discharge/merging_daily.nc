...
cmd     += 'python nc_basin_merge.py %s %s/%i_to_%i/ %i %i-01-01 %i-12-31 selected %s & ' %(input_folder[len(year_int)-1], output_folder, year_int[len(year_int)-1], last_year,            num_of_cores, year_int[len(year_int)-1], last_year           , netcdf_file_name)
cmd     += 'wait'
print cmd
os.system(cmd)
